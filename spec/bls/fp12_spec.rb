...
RSpec.describe 'bls12-381 Fp12' do
  ...
  it 'Fp12 equality' do
    NUM_RUNS.times do
      nums = 12.times.map { rand(1..BLS::Fp::ORDER) }
      a = BLS::Fp12.from_tuple(nums)
      b = BLS::Fp12.from_tuple(nums)
      expect(a).to eq(b)
    end
  end
  ...
  it 'Fp12 non-equality' do
    NUM_RUNS.times do
      num1 = 12.times.map { rand(1..BLS::Fp::ORDER) }
      num2 = 12.times.map { rand(1..BLS::Fp::ORDER) }
      a = BLS::Fp12.from_tuple(num1)
      b = BLS::Fp12.from_tuple(num2)
  ...
  it 'Fp12 multiplication and add equality' do
    NUM_RUNS.times do
      a = create_fp12_items(1)
      expect(a * 0).to eq(BLS::Fp12::ZERO)
      expect(a * BLS::Fp12::ZERO).to eq(BLS::Fp12::ZERO)
      expect(a * 1).to eq(a)
      expect(a * BLS::Fp12::ONE).to eq(a)
  ...
  it 'Fp12 division with zero equality' do
    NUM_RUNS.times do
      a = create_fp12_items(1)
      expect(BLS::Fp12::ZERO / a).to eq(BLS::Fp12::ZERO)
    end
  end
  ...
  it 'Fp12 addition with zero equality' do
    NUM_RUNS.times do
      a = create_fp12_items(1)
      expect(a + BLS::Fp12::ZERO).to eq(a)
    end
  end
  ...
  it 'Fp12 minus zero equality' do
    NUM_RUNS.times do
      a = create_fp12_items(1)
      expect(a - BLS::Fp12::ZERO).to eq(a)
      expect(a - a).to eq(BLS::Fp12::ZERO)
    end
  end
  ...
  it 'Fp12 minus and negative equality' do
    NUM_RUNS.times do
      a, b = create_fp12_items(2)
      expect(BLS::Fp12::ZERO - a).to eq(a.negate)
  ...
  it 'Fp12 negative equality' do
    NUM_RUNS.times do
      a = create_fp12_items(1)
      expect(a.negate).to eq(BLS::Fp12::ZERO - a)
