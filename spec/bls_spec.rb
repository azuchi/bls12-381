...
RSpec.describe 'bls12-381' do
  ...
  context 'Public key is G2 and Signature is G2' do
    ...
    context 'invalid' do
      ...
      # The signature is on the curve over Fp but outside G1, so it is rejected while
      # being deserialized instead of only failing verification.
      it 'rejects a signature that is not in the prime-order subgroup' do
        expect do
          BLS::PointG1.from_hex(
            "ace9fcdd9bc977e05d6328f889dc4e7c99114c737a494653cb27a1f55c06f4555e0f160980af5ead098acc195010b2f8")
        end.to raise_error(BLS::PointError, /prime-order subgroup/)
      end
    end
  end
